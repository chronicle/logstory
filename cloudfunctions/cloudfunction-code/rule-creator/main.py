...
def batch_create_rules():
  ...
  for rule_id, blob in existing_modified.items():
    ...
    if file_object is not None:
      with file_object.open("r") as f:
        ...
        uri_to_post = (
            f"{DETECTION_API_BASE_URL}/v2/detect/rules/{rule_id}:createVersion"
        )
        response = http_client.request(
            uri_to_post, "POST", body=json.dumps(request_body)
        )
        print(response)
  ...
  if failed:
    print(
        f"Failed to create {len(failed)} rule(s):\n",
    ...
    for existing_rule in existing_rules["rules"]:
      if blob.name == existing_rule["ruleName"]:  # overlap
        ...
        if "liveRuleEnabled" not in existing_rule:
          uri_to_enable = (
              f"{DETECTION_API_BASE_URL}/v2/detect/rules/{rule_id}:enableLiveRule"
          )
          enable_rule = http_client.request(uri_to_enable, "POST")
          print("Rule enabled: " + rule_name)
        if "alertingEnabled" not in existing_rule:
          uri_to_alerting = (
              f"{DETECTION_API_BASE_URL}/v2/detect/rules/{rule_id}:enableAlerting"
          )
