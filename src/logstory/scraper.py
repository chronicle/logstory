  ...
  if raw_record:
    try:
      if isinstance(raw_record, str):
        decoded = base64.b64decode(raw_record.encode()).decode(
            "utf-8", errors="ignore"
        )
      else:
        decoded = base64.b64decode(raw_record).decode(
            "utf-8", errors="ignore"
        )
...
def _resolve_secops_config(
  ...
  resolved = {
      ...
      "default_api_version": api_version or config.get(
          "api_version", "v1alpha"
      ),
  }
...
def scrape_to_usecase_logic(
  ...
  if name:
    ...
    usecase_name = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M")
  ...
  if ingestion_label:
    search_start, search_end = _resolve_time_range(
    ...
    active_log_types = log_types if log_types is not None else ",".join(resolved_log_types)
  ...
  print(f"Searching raw logs from {resolved_start.isoformat()} to {resolved_end.isoformat()}...")

  matches = []
  time_slices = _generate_time_slices(
      resolved_start, resolved_end, chunk_size
  )
  print(
  ...
  for idx, (slice_start, slice_end) in enumerate(time_slices):
    ...
    if active_log_types:
      search_kwargs["log_types"] = [
          lt.strip() for lt in active_log_types.split(",")
      ]
    ...
    if total_scraped >= max_events:
      print(
          f"  Safety ceiling limit of {max_events} events reached. "
          "Stopping loop early."
      )
  ...
  if not package_usecase_link.exists():
    try:
      ...
      print(f"Created package symlink: {package_usecase_link} -> ../../../usecases/{usecase_name}")
    except Exception as e:
  ...
  yaml_lines = [
      ...
      (
          "description: "
          f'"Usecase for raw logs scraped using query: {escaped_query}"'
      ),
      "tags:",
      "  - scraped-logs",
      "  - secops-scraper",
  ...
  compiler_script = (
      package_dir / "../../usecases/generate_usecase_docs.py"
  ).resolve()
