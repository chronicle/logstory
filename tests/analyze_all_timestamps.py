...
from test_timestamp_patterns import (
  analyze_pattern_matches,
  load_timestamp_patterns
)
...
def find_best_patterns(results: Dict[str, Dict]) -> Dict[str, List[Dict]]:
  ...
  for log_file, file_results in results.items():
    ...
    if best_log_type and max_matches > 0:
      ...
      matching_patterns = [
        m for m in file_results[best_log_type]['matches']
        if m['match_count'] > 0
      ]
        ...
        for j in range(i + 1, len(all_matches)):
          if all_matches[i]['end'] > all_matches[j]['start']:
            ...
            report.append(
              ...
              f"overlaps {all_matches[j]['pattern']} [{all_matches[j]['start']}:{all_matches[j]['end']}]"
            )
...
def main():
  ...
  parser.add_argument(
    'yaml_file',
    type=Path,
    help='Path to YAML file containing timestamp patterns'
  )
  parser.add_argument(
    'log_dir',
    type=Path,
  ...
  parser.add_argument(
    '--pattern',
    default='*.log',
    help='File pattern for log files (default: *.log)'
  )
  parser.add_argument(
    '--output',
    type=Path,
