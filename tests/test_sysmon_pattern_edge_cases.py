...
class TestSysmonPatternEdgeCases(unittest.TestCase):
  ...
  def setUp(self):
    """Set up test data."""
    yaml_path = Path(__file__).parent.parent / "src/logstory/logtypes_events_timestamps.yaml"
  ...
  def test_millisecond_truncation(self):
    ...
    self.assertEqual(captured, "2024-01-25 19:53:06", 
  ...
  def test_improved_pattern_with_milliseconds(self):
    ...
    improved_pattern = r'("UtcTime"\s*:\s*"?)(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(?:\.\d{3})?)'
  ...
  def test_epoch_vs_datetime_distinction(self):
    ...
    self.assertIn('EventReceivedTime', epoch_fields, "EventReceivedTime should be epoch")
  ...
  def test_json_vs_xml_format_handling(self):
    ...
    xml_patterns = [p for p in self.sysmon_patterns if '"' not in p['pattern'] or 'optional' in p.get('description', '')]
  ...
  def test_pattern_specificity(self):
    ...
    for test_string in false_positive_cases:
      ...
      self.assertFalse(matched, f"No pattern should match non-timestamp field: {test_string}")
